...
<body>
	...
	<form action="${ctp }/customer/list" method="POST">
		<table class="query_form_table" border="0" cellPadding="3"
			...
			<tr>
				<th>客户名称</th>
				<td><input type="text" name="search_LIKE_name" /></td>
				<th>地区</th>
				<td><select name="search_EQ_region">
			...
			<tr>
				...
				<th>客户等级</th>
				<td><select name="search_EQ_level">
						<option value="">全部</option>
				...
				<th>状态</th>
				<td><select name="search_EQ_state">
						<option value="">全部</option>
		...
		<c:if test="${page != null && page.totalElements > 0 }">
				...
				<c:forEach var="customer" items="${page.content }">
					<tr>
						...
						<td class="list_data_op"><img
							onclick="window.location.href='${ctp}/customer/create?id=${customer.id}'"
							title="编辑" src="${ctp }/static/images/bt_edit.gif"
							class="op_button" alt="" /> <img
							onclick="window.location.href='${ctp}/contact/list?customerid=${customer.id }'"
							...
							onclick="window.location.href='${ctp}/order/list?customerid=${customer.id }'"
			...
			<tags:page list="customer/list" page="${page}"></tags:page>
		</c:if>
		<c:if test="${page == null || page.totalElements == 0 }">
			没有任何数据
		</c:if>
