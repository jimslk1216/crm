...
<body class="main">
	<form action="${ctp }/plan/list" method="post">
		...
		<c:if test="${page != null && page.totalElements > 0 }">
				...
				<c:forEach var="chance" items="${page.content }">
					<tr>
						...
						<td class="list_data_op"><c:if test='${chance.status==2}'>
								<img
									onclick="window.location.href='${ctp }/plan/make?id=${chance.id}'"
								...
								<img id="finish-${chance.id}"
									onclick="${ctp }/plan/finish/${chance.id}" title="开发成功"
									src="${ctp }/static/images/bt_yes.gif" class="op_button" />

							</c:if> <c:if test='${chance.status!="2"}'>
								<img id="detail-${chance.id}"
			...
			<tags:page page="${page }" list="plan/chance/list" />
