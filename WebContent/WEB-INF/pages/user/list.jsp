...
<script type="text/javascript">
	$(function() {
		...
		$("img[id^='toupdate_']").click(function() {
			$(this).attr("onclick").submit();
		});
				...
				<c:forEach var="user" items="${page.content }">
					<tr>
						...
						<td class="data_cell" style="text-align: center;">
							${user.name}</td>
						<td class="data_cell">${user.enabled == 1 ? "有效" : "无效"}</td>
						<td class="data_cell"><img id="delete_${user.id }"
							onclick="${ctp}/user/delete/${user.id}" title="删除"
							src="${ctp}/static/images/bt_del.gif" class="op_button" /> <img
							id="toupdate_${user.id }"
							onclick="window.location.href='${ctp}/user/create/${user.id}'"
							class="op_button" src="${ctp}/static/images/bt_edit.gif"
							title="编辑" /></td>
					</tr>
				</c:forEach>
			</table>
			<tags:page list="user/list" page="${page }"
				queryString="${queryString }"></tags:page>
		</c:if>
		<c:if test="${page == null || page.totalElements == 0 }">
			没有任何数据
		</c:if>
